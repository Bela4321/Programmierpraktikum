...
public class KnapsackFitnessEvaluator implements ga.framework.operators.FitnessEvaluator {
    ...
    public void evaluate(List<Solution> population) {
        population.stream().forEach(solution -> { if (solution instanceof KnapsackSolution){solution.setFitness((double)((KnapsackSolution) solution).weight/((KnapsackSolution) solution).capacity);}
